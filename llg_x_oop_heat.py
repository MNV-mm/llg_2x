...
class llg2_solver:
    ...
    def __init__(self, Lx, Ly, Nx, Ny, mesh_2d):
    ...
    def norm_vec(self, u):
        v = u.copy()
        N = u.x.array.size//3
        #v_vec = np.reshape(v.x.array, (self.Nx+1, self.Ny+1, 3))
        v_vec = np.reshape(v.x.array, (N, 3))
        
        #norm = np.sqrt(v_vec[:,:,0]**2 + v_vec[:,:,1]**2 + v_vec[:,:,2]**2)
        norm = np.sqrt(v_vec[:,0]**2 + v_vec[:,1]**2 + v_vec[:,2]**2)
        #coords = self.mesh_2d.geometry.x.reshape((self.Nx+1, self.Ny+1, 3))
        coords = self.mesh_2d.geometry.x.reshape((N, 3))
        if self.aniso_func == True:
            #v_vec = v_vec/np.expand_dims(norm, axis = 2)*np.expand_dims(1-self.delta_Ms/self.Ms*np.exp(-0.5*(coords[:,:,0]**2 + coords[:,:,1]**2)/self.sigma**2), axis = 2)
            v_vec = v_vec/np.expand_dims(norm, axis = 1)*np.expand_dims(1-self.delta_Ms/self.Ms*np.exp(-0.5*(coords[:,0]**2 + coords[:,1]**2)/self.sigma**2), axis = 1)
        if self.aniso_func == False:
            #v_vec = v_vec/np.expand_dims(norm, axis = 2)
            v_vec = v_vec/np.expand_dims(norm, axis = 1)
    ...
    def h_rest(self):
        ...
        # self.Ku_func = fem.Function(self.FS_1d)
        # self.Ku_func.interpolate(fem.Expression(1-self.delta_Ku/self.kku*exp(-0.5*(x[0]**2+x[1]**2)/self.sigma**2), self.FS_1d.element.interpolation_points()))
        ...
        vec_total = as_vector((oo, oo, self.Ku_func*m3)) #+ vec
    ...
    def set_params(self, alpha = 1, kku = 1000, A_ex = 9.5*10**(-8), Ms = 4, aniso_func = False, delta_Ms = 0, delta_Ku = 0, sigma = 1):
        ...
        self.aniso_func = aniso_func
        
        self.delta_Ms = delta_Ms
        self.delta_Ku = delta_Ku
        self.sigma = sigma
        
        x = SpatialCoordinate(mesh_2d)
        
        self.Ku_func = fem.Function(self.FS_1d, name = 'Ku_func')
        
        if aniso_func == True:
            #self.Ku_func = fem.Function(self.FS_1d, name = 'Ku_func')
            self.Ku_func.interpolate(fem.Expression(1-self.delta_Ku*exp(-0.5*(x[0]**2+x[1]**2)/self.sigma**2), self.FS_1d.element.interpolation_points()))
        elif aniso_func == False:
            #self.Ku_func = fem.Function(self.FS_1d, name = 'Ku_func')
            self.Ku_func.x.array[:] = np.ones(self.Ku_func.x.array.shape)
        else:
            raise NotImplementedError()
    ...
    def solve(self, each_idx_write = 10):
        ...
        ku_file = XDMFFile(mesh_2d.comm, self.route_0 + "ku.xdmf", "w")
        ku_file.write_mesh(self.mesh_2d)
        ku_file.write_function(self.Ku_func, self.T)
        ku_file.close()
        
        derivative_func = fem.Function(self.FS, name = 'derivative_func')
...
llg_solver = llg2_solver(Lx, Ly, Nx, Ny, mesh_2d)

x = SpatialCoordinate(mesh_2d)
ub_expr = fem.Expression(as_vector((
    sin_0*(1-delta_Ms/Ms*exp(-0.5*(x[0]**2+x[1]**2)/sigma**2)), 
    0, 
    cos_0*(1-delta_Ms/Ms*exp(-0.5*(x[0]**2+x[1]**2)/sigma**2)))), 
    llg_solver.FS.element.interpolation_points())

# 1*(1-delta_Ms/Ms*exp(-0.5*(x[0]**2+x[1]**2)/sigma**2))
# fem.Constant(llg_solver.mesh_2d, sin_0)

llg_solver.set_in_cond(ub_expr, in_type='new', path_to_sol = None)

llg_solver.set_params(alpha = 1E-4, kku = Ku, A_ex = 9.5*10**(-8), Ms = Ms, aniso_func = True, 
                      delta_Ms = delta_Ms, delta_Ku = delta_Ku, sigma = sigma)

llg_solver.set_h_ext(h_x = h_x)

dt = 5*0.001
N_f = 1000

llg_solver.set_comp_params(dt, N_f, route_0 = '/media/mnv/T7/HEAT/test/graphs/')

llg_solver.e_field_from_ps(x0 = 0, y0 = 2, r0 = 0.00002, U0 = -0*2*10/3/50, gamma_me = 1E-6)
